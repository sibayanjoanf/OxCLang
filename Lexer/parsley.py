...
class Parser:
    ...
    def parse_body(self):
        ...
        if current in ['int', 'float', 'char', 'string', 'bool', 'gust', 'wind', 'inhale', 'exhale', '++', '--',
            ...
            self.error(f"Invalid body start. Expected statements, got '{current}'")
    ...
    def parse_or_tail(self):
        ...
        if current == '||':
            ...
            if next_tok not in self.FIRST_PRIMARY and not (next_tok and next_tok.startswith('id')):
                self.error(f"Expected relational expression or boolean after '||' operator, got '{next_tok}'")
                raise StopIteration
    ...
    def parse_and_tail(self):
        ...
        if current == '&&':
            ...
            self.error(f"Expected '&&' or '||' or '~' or ',' , got '{current}'")
    ...
    def parse_arith_tail(self):
        ...
        if current in ['+', '-']:
            ...
            next_tok = self.peek()
            if next_tok not in self.FIRST_PRIMARY and not (next_tok and next_tok.startswith('id')):
                self.error(f"Expected expression after '{arith_op1_node.children[0].value}' operator, got '{next_tok}'")
                raise StopIteration
            ...
            self.error(f"Expected operators or '~', got '{current}'")
    ...
    def parse_term_tail(self):
        ...
        if current in ['*', '/', '%']:
            ...
            next_tok = self.peek()
            if next_tok not in self.FIRST_PRIMARY and not (next_tok and next_tok.startswith('id')):
                self.error(f"Expected expression after '{arith_op2_node.children[0].value}' operator, got '{next_tok}'")
                raise StopIteration
            ...
            self.error(f"Expected terminator '~' or continuation of expression, got '{current}'.")
            raise StopIteration
